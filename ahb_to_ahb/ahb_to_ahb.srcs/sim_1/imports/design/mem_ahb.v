...
//you should learn mem_ahb carefully,
...
(
   ...
   always @ (posedge HCLK or negedge HRESETn) begin
       if (HRESETn==0) begin
           ...
           if (T_ENABLED) begin
               T_ADDR <= #1 HADDR[ADD_WIDTH-1:0];
               T_BE   <= #1 byte_enable(HADDR[1:0], HSIZE);
               T_WR   <= #1 HWRITE;
               HRDATA <= #1 mem[HADDR[ADD_WIDTH-1:2]]; //HRDATA get data, as soon as T_ENABLED = 1.
           end else begin
               T_BE   <= #1 4'h0;
               T_WR   <= #1 1'b0;
           end
           if (T_WR) begin
               T_DATA[ 7: 0] <= #1 (T_BE[0]) ? HWDATA[ 7: 0]
                                          : HRDATA[ 7: 0];
               T_DATA[15: 8] <= #1 (T_BE[1]) ? HWDATA[15: 8]
                                          : HRDATA[15:8];
               T_DATA[23:16] <= #1 (T_BE[2]) ? HWDATA[23:16]
                                          : HRDATA[23:16];
               T_DATA[31:24] <= #1 (T_BE[3]) ? HWDATA[31:24]
                                          : HRDATA[31:24];
               T_BE_D <= #1 T_BE;
               T_WR_D <= #1 T_WR;
           end else begin
           ...
           if (T_WR_D) begin
               mem[T_ADDRw[ADD_WIDTH-1:2]] <= #1 T_DATA;
           end
           T_ADDRw <= #1 T_ADDR;
       end
   end
...
$display("%m ERROR: DELAY parameter should be less than 0x1F, but 0x%x", DELAY);
   ...
   //control DELAY. added by huanglc at 20240312
   always @ (posedge HCLK or negedge HRESETn) begin
       if (HRESETn==0) begin
           HREADYout <= 1'b1;
           count     <= 'h0;
           ...
           case (state)
               IDLE: begin
                     if (T_ENABLED&&(DELAY!=0)) begin   //in sim_define.vh, DELAY = 1.
                         HREADYout <= #1 1'b0;
                         count     <= #1 'h1;
                         state     <= #1 WAIT;
                     end
